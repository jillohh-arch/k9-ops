...
describe("Gate 5D.8 — Web Conflict Reader + Emulator", () => {
  ...
  it("NÃO escolhe latest-wins — todos ativos são reportados no conflict", () => {
    ...
    // Plano "antigo" e plano "recente": NutritionPlan não possui campo de
    // criação, então a distinção temporal não é observável pelo reader.
    // É exatamente esse o ponto do teste — não existe critério latest-wins.
    const oldPlan = makeEmulatorPlan("plan-old-1234", dogId, 1, "active");
    const newPlan = makeEmulatorPlan("plan-new-5678", dogId, 1, "active");
