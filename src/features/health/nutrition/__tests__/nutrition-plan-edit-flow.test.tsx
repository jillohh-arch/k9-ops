...
describe("Gate 5D.3 — Edit Nutrition Plan Administrative Fields", () => {
  ...
  describe("CTA Availability across Read States", () => {
    it("does not silently select the first K9 and delegates selection to the URL owner", () => {
      ...
      render(<NutritionPlanManagement onDogIdChange={onDogIdChange} />);

      const selector = screen.getByLabelText("Selecionar K9");
      expect(selector).toHaveValue("");
    expect(
      screen.getByRole("heading", { name: "Selecione um K9" }),
    ).toBeInTheDocument();
      expect(mockPlanState).toHaveBeenCalledWith("");
      fireEvent.change(selector, { target: { value: "dog-1" } });
      expect(onDogIdChange).toHaveBeenCalledWith("dog-1");
    });

    it("renders an explicit invalid-K9 state instead of opening another dog", () => {
      mockPlanState.mockReturnValue({
        status: "empty",
        dogId: "",
        generation: 1,
      ...
      render(<NutritionPlanManagement initialDogId="unknown-dog" />);

      expect(screen.getByText("K9 inválido")).toBeInTheDocument();
      expect(screen.queryByRole("button", { name: /Novo Plano Alimentar/i }))
  ...
  describe("Stale Form & Revision Integrity", () => {
    it("suppresses only the reconciliation snapshot while UPDATE is executing", () => {
      const view = render(
        <NutritionPlanEditDialog
          open={true}
          onClose={vi.fn()}
      ...
      view.rerender(
        <NutritionPlanEditDialog
          open={true}
          onClose={vi.fn()}
          plan={{ ...sampleActivePlan, revision: 4 }}
          dogName="Thor"
        />,
      );
      expect(screen.queryByTestId("stale-form-warning")).not.toBeInTheDocument();
    });

    it("suppresses the immediate old snapshot after UPDATE success", () => {
      const view = render(
        <NutritionPlanEditDialog
          open={true}
          onClose={vi.fn()}
    ...
    it("ends protection after failure and blocks the externally newer revision", () => {
      const view = render(
        <NutritionPlanEditDialog
          open={true}
          onClose={vi.fn()}
    ...
    it("invalidates the dialog when the selected K9 plan changes", () => {
      const view = render(
        <NutritionPlanEditDialog
          open={true}
          onClose={vi.fn()}
      ...
      view.rerender(
        <NutritionPlanEditDialog
          open={true}
          onClose={vi.fn()}
          plan={{ ...sampleActivePlan, id: "plan-dog-2", dogId: "dog-2" }}
          dogName="Lua"
        />,
      );
      expect(screen.getByTestId("stale-form-warning")).toBeInTheDocument();
