...
describe("CREATE Mutation", () => {
  ...
  describe("retryCreate", () => {
    ...
    it("waits for explicit retry and reuses the unavailable operationId", async () => {
      const { result } = renderHook(() => useNutritionPlanMutations({ functions: mockFunctions }));
      ...
      const firstOperationId = mockExecuteCreate.mock.calls[0][1].operationId;
      ...
      const secondOperationId = mockExecuteCreate.mock.calls[1][1].operationId;
      expect(secondOperationId).toBe(firstOperationId);
      expect(secondOperationId).toBe(opId);
      expect(result.current.createState.status).toBe("success");
    });
