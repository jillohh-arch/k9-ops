...
export function NutritionPlanManagement({
  ...
  const requestedDogExists = entityDogs.some((dog) => dog._id === requestedDogId);
  const currentDogId =
    requestedDogId && requestedDogExists ? requestedDogId : "";
  const invalidDogId =
    !dogsLoading && Boolean(requestedDogId) && !requestedDogExists;
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <div className="flex items-center gap-2 rounded-2xl border border-cyan-500/20 bg-slate-900/90 p-2 px-3 shadow-sm">
              ...
              >
                <option value="">
                  {entityDogs.length === 0 ? "Nenhum K9 disponível" : "Selecione um K9"}
                </option>
                {entityDogs.length === 0 ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-slate-400 mt-0.5">
                CREATE, UPDATE administrativo, REPLACE estrutural e CANCEL usam
                operações compostas com validação de capability.
              </div>
            </div>
          </div>
      ...
      {invalidDogId && (
        <div className="flex items-start gap-4 rounded-2xl border border-amber-500/40 bg-amber-950/20 p-6 text-amber-100">
          <AlertCircle className="mt-0.5 h-6 w-6 shrink-0 text-amber-400" />
          <div>
            <h4 className="font-bold">K9 inválido</h4>
      ...
      {missingDogId && entityDogs.length > 0 && (
        <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-8 text-center">
          <h4 className="font-bold text-slate-100">Selecione um K9</h4>
          <p className="mt-2 text-sm text-slate-400">
            Nenhum plano é aberto automaticamente sem um dogId válido na URL.
          </p>
        </div>
      )}

      {/* ESTADOS DO READ MODEL */}
      {currentDogId && planState.status === "loading" && <NutritionPlanLoadingSkeleton />}

      {currentDogId && planState.status === "conflict" && (
      ...
      {currentDogId && planState.status === "error" && (
        <div
      ...
      {currentDogId && planState.status === "empty" && (
      ...
      {currentDogId && planState.status === "degraded" && (
      ...
      {currentDogId && planState.status === "canonical" && planState.activePlan && (
        <NutritionPlanActiveView
      ...
      {currentDogId && planState.status === "legacy" && planState.activePlan && (
        <NutritionPlanLegacyView
          plan={planState.activePlan as LegacyNutritionPlanView}
          dogName={dogName}
          canManage={canManage}
