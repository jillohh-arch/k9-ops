...
/**
 * The create-and-activate callable accepts expectedActivePlanId and expectedActiveRevision
 * to execute atomic plan replacement.
 */
export const REPLACE_ATOMIC_CONTRACT_AVAILABLE = true;
...
export function shouldShowNutritionReplaceStale(input: {
  ...
  return (
    ...
    input.mutationStatus !== "success" &&
    (input.planId !== input.snapshotPlanId ||
      input.planRevision !== input.snapshotRevision ||
...
export function NutritionPlanReplaceDialog({
  ...
  const handleConfirmReplacement = async () => {
    ...
    if (!REPLACE_ATOMIC_CONTRACT_AVAILABLE) {
      setLocalError(
        "Substituição indisponível: o backend ainda não valida atomicamente o plano ativo e sua revisão esperada.",
      );
      return;
    }
    if (isStale) {
      setLocalError(
        "O plano ativo mudou durante a revisão. Volte e recarregue a versão atual antes de confirmar.",
      );
      return;
    }
    ...
    const command: CreateNutritionPlanCommand = {
      dogId: plan.dogId,
      expectedActivePlanId: snapshotPlanId,
      expectedActiveRevision: snapshotRevision,
      planData: {
    ...
    try {
      ...
      // The hook owns the sanitized error; the dialog must leave executing.
      setPhase("reviewing");
    }
  };
  ...
  const handleRetryReplacement = async () => {
    ...
    if (!REPLACE_ATOMIC_CONTRACT_AVAILABLE) {
      setLocalError(
        "Retry bloqueado até o backend validar atomicamente a revisão do plano substituído.",
      );
      return;
    }
    if (isStale) {
      setLocalError("O plano ativo mudou. Recarregue a versão atual antes do retry.");
      return;
    }
    setPhase("executing");
    try {
    ...
    >
      <div className="min-w-0 space-y-6 overflow-x-hidden">
        ...
        {!REPLACE_ATOMIC_CONTRACT_AVAILABLE && (
          <div
            data-testid="replace-atomic-contract-block"
            className="flex items-start gap-3 rounded-2xl border border-red-500/40 bg-red-950/30 p-4 text-xs text-red-200"
          >
            <ShieldAlert className="mt-0.5 h-5 w-5 shrink-0 text-red-400" />
            <div>
              <strong className="font-bold text-red-100">
                Substituição temporariamente bloqueada
              </strong>
              <p className="mt-1 text-red-200/80">
                A transação atual não recebe o plano ativo e a revisão esperada.
                A confirmação permanecerá desabilitada até existir validação atômica no backend.
              </p>
            </div>
          </div>
        )}
              ...
              {createState.error &&
                ...
                createState.error.retryable && (
                  <Button
                    type="button"
                    variant="danger"
                    onClick={handleRetryReplacement}
                    aria-describedby="replace-mutation-error"
                    disabled={
                      phase === "executing" ||
                      isStale ||
                      !REPLACE_ATOMIC_CONTRACT_AVAILABLE
        ...
        {phase === "editing" && (
          ...
          >
            <fieldset disabled={isSuccess || isStale} className="min-w-0 space-y-6">
                  ...
                  {mealSchedule.map((slot) => (
                    ...
                    >
                      <div className="w-full sm:w-1/3">
                        ...
                        <select
                          aria-label={`Período da refeição ${slot.id}`}
                          value={slot.period}
                          onChange={(e) =>
                      ...
                      <div className="w-full sm:w-1/3">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <Input
                            aria-label={`Horário programado da refeição ${slot.id}`}
                            type="time"
                      ...
                      <div className="w-full sm:w-1/3">
                        ...
                        <Input
                          aria-label={`Quantidade em gramas da refeição ${slot.id}`}
                          type="number"
                          min="1"
                      ...
                      <div className="flex items-end justify-end sm:self-end pb-1">
                        <Button
                          ...
                          aria-label={`Remover refeição ${PERIOD_LABELS[slot.period]}`}
                      ...
                      >
                        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                          <div className="sm:col-span-2">
                            ...
                            <Input
                              aria-label={`Nome do suplemento ${supp.id}`}
                          ...
                          <div>
                            ...
                            <Input
                              aria-label={`Dose do suplemento ${supp.name || supp.id}`}
                              type="number"
                              placeholder="Ex: 2"
                              value={supp.dose}
                              min={0.1}
                          ...
                          <div>
                            ...
                            <select
                              aria-label={`Unidade do suplemento ${supp.name || supp.id}`}
                              value={supp.unit}
                              onChange={(e) =>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 items-end">
                          <div className="sm:col-span-2">
                            ...
                            <Input
                              aria-label={`Frequência do suplemento ${supp.name || supp.id}`}
                          ...
                          <div className="sm:col-span-2 flex items-center justify-between gap-2">
                            <div className="w-full">
                              ...
                              <Input
                                aria-label={`Instruções do suplemento ${supp.name || supp.id}`}
                            ...
                            <Button
                              ...
                              aria-label={`Remover suplemento ${supp.name || "sem nome"}`}
        ...
        {phase === "executing" && !isSuccess && !isError && (
          ...
          >
            <RefreshCw
              aria-hidden="true"
              className="h-12 w-12 text-cyan-400 animate-spin motion-reduce:animate-none"
            />
