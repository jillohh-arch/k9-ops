 ...
 * Nutrition plan mutation state machine.
 *
 * Not wired to any UI in this phase: no button, dialog or management surface
 * consumes it yet. It exists so the operationId / revision / retry guarantees
 * can be proven before the first write affordance ships (B.4+).
 *
 * The prepare → execute split is what makes idempotency real: `prepare*` mints
 * the operationId once and freezes the command; `execute*` and `retry*` send
 * that frozen intent unchanged. A retry therefore reaches the backend with the
 * SAME operationId, which the backend treats as a safe replay (observable as
 * `wasNoOp`) instead of creating a second plan. Collapsing prepare and execute
 * into one call would regenerate the id per attempt and turn a retry into a
 * duplicate mutation.
 *
 * Revision expectations travel inside the frozen intent, so a retry never
 * silently re-reads a fresher revision. revision-conflict and
 * idempotency-conflict surface to the caller rather than being "fixed" here.
 */
